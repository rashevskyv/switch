...
1. Установите свежий {% include abbr/kefir_addr.txt %}
...
1. **Если у вас был EmuNAND** (если не было, то достаточно просто установки кефира), на заставке кефира **нажмите кнопку понижения громкости**, чтобы попасть в hekate 	
	...
	1. Нажмите "**ОК**"
		...
		* Чтобы попасть в пиратскую прошивку, запустите **hekate -> Launch -> Atmosphere**. Будет запущен Emunand. Если Emunand запускается с ошибкой, либо долго висит на черном экране, обратитесь к разделу [проблемы и их решения](#проблемы-и-их-решения){:target="_blank"} внизу страницы
...
1. Скопируйте папку `emuMMC/eMMC` из корня вашей карты памяти на ПК. Это ваш эмунанд. Он идентичен бекапу, поэтому нет никакого смысла создавать бекап отдельно 
	* Если такой папки нет, то у вас нет EmuNAND, тогда сделайте бекап согласно это [инструкции](backup-nand){:target="_blank"}
...
1. Попробуйте запустить SysNAND (**hekate -> Launch -> Stock**). Если он не запускается, обратитесь к разделу [проблемы и их решения](#проблемы-и-их-решения){:target="_blank"} внизу страницы
...
1. Вставьте карту памяти приставки в ПК 
1. Скачайте [reBoot_20210418.zip](files/reBoot_20210418.zip){:target="_blank"}
1. [Скачайте прошивку](https://darthsternie.net/switch-firmwares/){:target="_blank"}, соответствующую текущей версии прошивки вашего emunand
	* Важно точно знать какая прошивка на вашем emunand! Попробуйте запустить SXOS и посмотреть какая версия прошивки в вашем EmuNAND. Для этого скачайте [SX_OS 3.1.0](https://drive.google.com/file/d/1PZ7w1J5PAQf4CrUWfzfA1mVnCNeD13RP){:target="_blank"} и поместите скачанный `boot.dat` в корень карты памяти с заменой, затем вставьте КП в консоль и включите приставку 
1. Распакуйте архив с прошивкой в папку `firmware` на вашем ПК 
1. Поместите файлы из архива [reBoot_20210418.zip](files/reBoot_20210418.zip){:target="_blank"} в папку `firmware`
