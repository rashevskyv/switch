...
1. Попробуйте запустить {% include abbr/ru/hekate.txt abbr="hekate" %}. Если получилось, возможно вы пытались откатить прошивку из-за чего случился конфликт между версией прошивки и сожжёнными фьюзами. Попробуйте запустить **Full Stock** и [обновить прошивку до последней версии](update-fw){:target="_blank"}, или отключить Emunand (hekate > emuMMC > Change emummc > Disable) и обновить прошивку через настройки
1. [Восстановите бекап](backup-nand#восстановление-резервной-копии){:target="_blank"}, если таковой имеется
1. Сделайте [восстановление прошивки](downgrade_fw){:target="_blank"}
