...
1. Вставьте карту памяти в консоль и запустите [{% include abbr/ru/cfw.txt abbr="CFW" %}](/ru/cfw){:target="_blank"}
1. Запустите DBI через [HBL](/ru/hbl){:target="_blank"}
...
**О:** Для переустановки тикетов, запустите [tinfoil](/ru/games){:target="_blank"}, перейдите на вкладку "**Тикеты**", и для каждой игры проделайте следующее:
1. На игре нажмите (Y)
1. Нажмите (R)
