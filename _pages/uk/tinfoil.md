...
1. Вставьте карту памяти в консоль и запустите [{% include abbr/uk/cfw.txt abbr="CFW" %}](/uk/cfw){:target="_blank"}
1. Запустите DBI через [HBL](/uk/hbl){:target="_blank"}
...
1. Перейдите в нужный раздел, выберите игру и нажмите {% include inc/uk/btn.txt btn="A" %}
	* В разделе **Новые игры** находятся все игры, которые есть на сервере
		* Для поиска нажмите {% include inc/uk/btn.txt btn="+" %} на джойконе, находясь на строке **Новые игры** в левой части меню
...
1. Нажмите {% include inc/uk/btn.txt btn="B" %}, чтобы свернуть появившееся окно
...
1. После того, как выбранная игра скачается, выйдите из программы кнопкой {% include inc/uk/btn.txt btn="HOME" %}
...
**О:** Для переустановки тикетов, запустите [tinfoil](/uk/games){:target="_blank"}, перейдите на вкладку "**Тикеты**", и для каждой игры проделайте следующее:
