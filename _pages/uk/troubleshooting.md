...
1. Убедитесь. что можете загрузиться в сток. Если это так, возможно вы установили какие-то системные модули, которые мешают загрузке. Для этого: 
	1. Запустите {% include abbr/ua/hekate.txt abbr="hekate" %} -> **Payloads** -> `TegraExplorer` -> `kefir-helper.te`
	1. Если после этого приставка заработала, ищите проблему в установленных модулях 
1. Переустановите {% include abbr/ua/kefir_addr.txt %} начисто. Для этого удалите с карты памяти всё, кроме папки `Nintendo` и `emummc` и ставьте кефир заново
1. Отформатируйте вашу карту памяти в [FAT32](https://format.customfw.xyz){:target="_blank"}. 
1. Переустановите {% include abbr/ua/kefir_addr.txt %} начисто. Для этого удалите с карты памяти всё, кроме папки `Nintendo` и `emummc` (или `sxos/emunand`) и ставьте кефир заново
1. Проверьте монтируется ли раздел `USER`. Для этого: 
	...
	1. Выберите **Browse EMUMMC** если не запускается {% include abbr/ua/emunand.md abbr="EmuNAND" %} или **Browse EMMC**, если не запускается {% include abbr/ua/sysnand.md abbr="SysNAND" %}
...
1. Попробуйте запустить {% include abbr/ua/hekate.txt abbr="hekate" %}. Если получилось, возможно вы пытались откатить прошивку из-за чего случился конфликт между версией прошивки и сожжёнными фьюзами. Попробуйте запустить **Full Stock** и [обновить прошивку до последней версии](update-fw){:target="_blank"}, или отключить Emunand (hekate > emuMMC > Change emummc > Disable) и обновить прошивку через настройки
1. [Восстановите бекап](backup-nand#восстановление-резервной-копии){:target="_blank"}, если таковой имеется
1. Сделайте [восстановление прошивки](downgrade_fw){:target="_blank"}
...
Если не помогает, переустановите {% include abbr/ua/kefir_addr.txt %} начисто. Для этого удалите с карты памяти всё, кроме папки Nintendo и emummc (или sxos/emunand) и ставьте кефир заново
...
Обновите {% include abbr/ua/kefir_addr.txt %}
{% endspoiler %}
...
Судя по всему вы используете EmuNAND и старый {% include abbr/ua/kefir_addr.txt %}. Обновите его. 
...
* Обновите {% include abbr/ua/kefir_addr.txt %} до актуальной версии 
...
Возможно, установлен ReverseNX. Удалите его. Так же попробуйте выключить приставку, запустить {% include abbr/ua/hekate.txt abbr="hekate" %}, перейдите в **Payloads** и выберите `fusee.bin`, после чего сразу зажмите одновременно обе кнопки громкости до тех пор, пока не попадёте в рекавери. После выключите приставку удерживая кнопку питания. Затем включите как обычно. 
