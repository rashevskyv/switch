...
---
lang: uk
В этом разделе только самые частые проблемы и их решения. Раздел, в котором собраны ответы на самые часто задаваемые вопросы находится здесь - [FAQ](/uk/faq){:target="_blank"}
...
1. Убедитесь. что можете загрузиться в сток. Если это так, возможно вы установили какие-то системные модули, которые мешают загрузке. Для этого: 
	1. Запустите {% include abbr/uk/hekate.txt abbr="hekate" %} -> **Payloads** -> `TegraExplorer` -> `kefir-helper.te`
	1. Если после этого приставка заработала, ищите проблему в установленных модулях 
1. Переустановите {% include abbr/uk/kefir_addr.txt %} начисто. Для этого удалите с карты памяти всё, кроме папки `Nintendo` и `emummc` и ставьте кефир заново
1. Отформатируйте вашу карту памяти в [FAT32](https://format.customfw.xyz){:target="_blank"}. 
1. Переустановите {% include abbr/uk/kefir_addr.txt %} начисто. Для этого удалите с карты памяти всё, кроме папки `Nintendo` и `emummc` (или `sxos/emunand`) и ставьте кефир заново
1. Проверьте монтируется ли раздел `USER`. Для этого: 
	...
	1. Выберите **Browse EMUMMC** если не запускается {% include abbr/uk/emunand.md abbr="EmuNAND" %} или **Browse EMMC**, если не запускается {% include abbr/uk/sysnand.md abbr="SysNAND" %}
	1. Перейдите в раздел **USER**. Если ошибок нет, то раздел USER монтируется и проблема не в нём
1. [Восстановите бекап](/uk/backup-nand#восстановление-резервной-копии){:target="_blank"}, если таковой имеется
1. Сделайте [сброс к заводским настройкам](/uk/system-wipe){:target="_blank"}
1. Сделайте [восстановление прошивки](/uk/downgrade_fw){:target="_blank"}
...
1. Попробуйте запустить {% include abbr/uk/hekate.txt abbr="hekate" %}. Если получилось, возможно вы пытались откатить прошивку из-за чего случился конфликт между версией прошивки и сожжёнными фьюзами. Попробуйте запустить **Full Stock** и [обновить прошивку до последней версии](/uk/update-fw){:target="_blank"}, или отключить Emunand (hekate > emuMMC > Change emummc > Disable) и обновить прошивку через настройки
1. [Восстановите бекап](/uk/backup-nand#восстановление-резервной-копии){:target="_blank"}, если таковой имеется
1. Сделайте [восстановление прошивки](/uk/downgrade_fw){:target="_blank"}
...
Если не помогает, переустановите {% include abbr/uk/kefir_addr.txt %} начисто. Для этого удалите с карты памяти всё, кроме папки Nintendo и emummc (или sxos/emunand) и ставьте кефир заново
...
Обновите {% include abbr/uk/kefir_addr.txt %}
{% endspoiler %}
...
Для корректной работы картриджей SysNAND и EmuNAND должны находится на одной версии прошивки. Если это не так, [обновите EmuNAND](/uk/update-fw). Если это не помогло, убедитесь, что в hekate у вас отключен **NoGC** (**Options** -> **Auto NoGC** -> **OFF**)
...
Запустите **DBI** и выберите "**Clean Orphaned Files**" - все файлы, на которых нет ссылок в системной базе данных будут удалены. **Важно:** запускайте DBI только [в режиме апплета](/uk/hbl){:target="_blank"}, кефир должен быть свежим. Иначе магия не сработает. 
{% endspoiler %}

{% spoiler Не могу установить игры в NAND %}

Судя по всему вы используете EmuNAND и старый {% include abbr/uk/kefir_addr.txt %}. Обновите его. 
...
* Обновите {% include abbr/uk/kefir_addr.txt %} до актуальной версии 
* Удалите tinfoil, обновите {% include abbr/uk/kefir_addr.txt %}, установите tinfoil из папки `games` на карте памяти 
...
Возможно, установлен ReverseNX. Удалите его. Так же попробуйте выключить приставку, запустить {% include abbr/uk/hekate.txt abbr="hekate" %}, перейдите в **Payloads** и выберите `fusee.bin`, после чего сразу зажмите одновременно обе кнопки громкости до тех пор, пока не попадёте в рекавери. После выключите приставку удерживая кнопку питания. Затем включите как обычно. 

{% endspoiler %}

{% spoiler Не запускается ни одна игра. При запуске идёт проверка, затем ошибка %}

Возможно, вы удалили тикеты всех игр. Для переустановки тикетов, запустите [tinfoil](/uk/games){:target="_blank"}, перейдите на вкладку "**Тикеты**", и для каждой игры проделайте следующее:
1. На игре нажмите {% include inc/uk/btn.txt btn="Y" %}
1. Нажмите {% include inc/uk/btn.txt btn="R" %}
...
Для переустановки тикетов, запустите [tinfoil](/uk/games){:target="_blank"}, перейдите на вкладку "**Тикеты**", и для каждой игры проделайте следующее:
1. На игре нажмите {% include inc/uk/btn.txt btn="Y" %}
1. Нажмите {% include inc/uk/btn.txt btn="R" %}
1. Включите опцию "**Установить Ticket**"
1. Нажмите "**Начать установку**"
...
Нет ни одной причины закидывать игры на карту памяти. Сразу [устанавливайте их](/uk/games){:target="_blank"} рекомендуемым способом по MTP через DBI!
...
Вам нужно установить драйвер exFAT. Для этого проделайте [безопасное обновление прошивки](/uk/update-fw){:target="_blank"}. Его нужно делать даже в том случае, если у вас и так стоит последняя версия взламываемой прошивки! Повторное обновление просто установит отсутствующий драйвер. А вообще, [переходите на FAT32](http://format.customfw.xyz){:target="_blank"}!
...
Имеется ввиду обновление игры. Скачайте его и установите как обычную игру. Так же сбросить проверку версии можно через DBI. Запустите программу, выберите **Browse installed applications**, нажмите {% include inc/uk/btn.txt btn="Y" %}, чтобы выделить все пункты, нажмите {% include inc/uk/btn.txt btn="A" %}, а затем выберите **Reset required version**. DBI должен быть [запущен из режима апплета](/uk/hbl){:target="_blank"}, кефир должен быть свежим. Иначе магия не сработает. 
