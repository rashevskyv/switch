...
Як правило, крім гри потрібно встановлювати останній патч і DLC, якщо вони виходили. Щодо патчу, потрібно ставити тільки останній, все що були перед ним ставити не треба. Останній патч завжди можна знайти в роздачі зі грою на [рутрекері](https://rutracker.org/forum/viewforum.php?f=1605){:target="_blank"}, або в [tinfoil](tinfoil){:target="_blank"}
...
* `0100BC300CB48000` - Title ID (потрібно знати для [використання модифікацій через {% include abbr/ua/lfs.txt abbr="LayeredFS" %}](#використання-перекладів-і-модифікацій){:target="_blank"})
...
1. По закінченню встановлення, натисніть {% include inc/ua/btn.txt btn="X" %}, щоб зупинити MTP-сервер і відмонтувати MTP-пристрій
1. Натисніть {% include inc/ua/btn.txt btn="+" %}, щоб закрити DBI
...
1. Виберіть ігри, які ви хочете встановити, кнопкою {% include inc/ua/btn.txt btn="X" %}
   * Натисніть {% include inc/ua/btn.txt btn="Y" %}, якщо потрібно вибрати всі або інвертувати вибір
...
1. Натисніть {% include inc/ua/btn.txt btn="B" %} для виходу з установника в папку
1. Натисніть {% include inc/ua/btn.txt btn="+" %} для закриття програми
...
1. Виберіть необхідні опції в "**Install options**"
   ...
   * **Delete after install** - значення **Yes** дозволить програмі автоматично видаляти файли після того, як вони будуть встановлені
1. Виберіть **Start install** для початку встановлення
   * Після того, як встановлення закінчиться, можете закрити програму кнопкою {% include inc/ua/btn.txt btn="HOME" %}
...
Більш детальна інструкція знаходиться [тут](tinfoil){:target="_blank"}
...
Для коректної роботи перекладів потрібен активний модуль {% include abbr/ua/lfs.txt abbr="LayeredFS" %}. Цей модуль активований за замовчуванням. В Atmosphere файли для LFS потрібно класти в папку `atmosphere/contents/`.
