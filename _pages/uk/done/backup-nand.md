...
* Встановлений свіжий {% include abbr/ua/kefir_addr.txt %}
...
1. Встановіть {% include abbr/ua/kefir_addr.txt %} за інструкцією, що знаходиться в його репозиторії
...
1. Виберіть "**Dump keys from SysNAND**"
    * Навігація по меню здійснюється кнопками гучності, а вибір кнопкою {% include inc/ua/btn.txt btn="POWER" %}
	* Дампінг почнеться негайно
1. Натисніть {% include inc/ua/btn.txt btn="POWER" %}, щоб повернутися в головне меню програми
...
1. Для підтвердження натисніть {% include inc/ua/btn.txt btn="POWER" %}
...
Цей спосіб працює тільки на приставках першої ревізії - {% include abbr/ua/erista.md %}
{: .notice--warning}
...
При проблемах з відновленням або відсутності бекапу можете спробувати [цю інструкцію](downgrade_fw){:target="_blank"}
...
* **П:** Приставка не запускається після відновлення бекапу 
	* **В:** Спробуйте [цей метод](downgrade_fw){:target="_blank"} для відновлення та підвищення версії прошивки

* **П:** Я втратив бекап, або бекап не відновлюється
	* **В:** Спробуйте [цей метод](downgrade_fw){:target="_blank"} для відновлення та підвищення версії прошивки. Якщо не допомогло, спробуйте [вручну, з видаленням всіх даних](downgrade_fw_manual){:target="_blank"}
