...
Деякі мобільні процесори NVIDIA Tegra, випущені до 2016 року, містять вразливість переповнення буфера в режимі відновлення BootROM ({% include abbr/ua/rcm.txt abbr="RCM" %}). Зловмисник з фізичним доступом до USB-порту пристрою і можливістю примусового перезавантаження пристрою в {% include abbr/ua/rcm.txt abbr="RCM" %}, може використовувати вразливість для виконання неперевіреного коду.
...
* Кнопки затиснуті не одночасно. Спробуйте затискати спочатку (VOL+), а потім, не відпускаючи її, натисніть {% include inc/ua/btn.txt btn="POWER" %}
...
{% include abbr/ua/dongle.txt abbr="Донгл" %} необхідно зарядити комплектним кабелем microUSB-USB, або вставити в **увімкнену** приставку
{: .notice--warning}

* Сам {% include abbr/ua/dongle.txt abbr="донгл" %}
* Встановлена на донглі {% include abbr/ua/hekate.txt abbr="hekate" %}
...
* Для запуску {% include abbr/ua/hekate.txt abbr="hekate" %} просто вставте {% include abbr/ua/dongle.txt abbr="донгл" %} в приставку, коли з'явиться сплешскрін кефіру, натисніть кнопку зменшення гучності
* Для запуску будь-якого іншого пейлоада читайте далі

#### Що знадобиться

* {% include abbr/ua/payload.txt abbr="Пейлоад" %}, який ви збираєтеся запускати
	* Для запуску {% include abbr/ua/hekate.txt abbr="hekate" %} вона повинна бути прошита в {% include abbr/ua/dongle.txt abbr="донгл" %}, додатково її нікуди не потрібно скидати
* Приставка повинна бути в [режимі RCM](fusee-gelee#частина-i---
lang: ukвхід-в-rcm){:target="_blank"}

Нижче описаний метод запуску взагалі будь-якого пейлоада за допомогою {% include abbr/ua/dongle.txt abbr="донгла" %}, на якому прошита {% include abbr/ua/hekate.txt abbr="hekate" %}
{: .notice--info}

1. Встановіть на приставку {% include abbr/ua/kefir_addr.txt %}
1. Помістіть {% include abbr/ua/payload.txt abbr="пейлоад" %}, який ви збираєтеся запускати в папку `bootloader/payloads` на карті пам'яті консолі та вставте картку в консоль
...
* {% include abbr/ua/payload.txt abbr="Пейлоад" %}, який ви збираєтеся запускати на приставці
...
Для автоматичного завантаження NXBoot і доставки пейлоада на консоль необхідно запустити програму [PayloadStarter](https://github.com/KnightOfSteel/payload_starter/releases){:target="_blank"} і вибрати потрібний {% include abbr/ua/payload.txt abbr="пейлоад" %}.
...
* {% include abbr/ua/payload.txt abbr="Пейлоад" %}, який ви збираєтеся запускати на приставці
* USB type-С на USB type-A кабель для підключення Switch до {% include abbr/ua/host.txt abbr="хосту" %}
