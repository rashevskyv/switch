...
В этом разделе только ответы на самые часто задаваемые вопросы. Раздел, в котором собраны самые частые проблемы и их решения находится здесь - [Проблемы и их решения](/uk/troubleshooting){:target="_blank"}
...
**О:** Изучите [глоссарий](/uk/#основные-термины){:target="_blank"}
...
**О:** В контексте этого руководства - сборник, состоящий из выбранного {% include abbr/uk/cfw.txt abbr="кастома" %}, необходимых программ и скриптов, которые все это установят правильным образом. Состав кефира и инструкцию по установке можно посмотреть в его [репозитории](https://github.com/rashevskyv/kefir/releases/latest){:target="_blank"}
...
**О:** Воспользуйтесь [этой инструкцией](/uk/usage){:target="_blank"}
{% endspoiler %}
...
**О:** Нет. После перезагрузки приставка всё равно перезагрузится в кастом. Для приставок, прошитых через Fusée Gelée после каждого холодного старта, {% include abbr/uk/payload.txt abbr="пейлоад" %} нужно будет пробрасывать заново. Для чипованных консолей этого не нужно, поскольку пейлоад пробрасывает сам чип. 

{% endspoiler %}

{% spoiler Можно ли доставать карту памяти при включенной приставке?  %}
**О:** Нет. Перезагрузите приставку через меню электропитание (удержание кнопки {% include inc/uk/btn.txt btn="POWER" %} -> **Параметры электропитания** -> **Перезагрузка**), чтобы попасть в {% include abbr/uk/hekate.txt abbr="hekate" %}. Оттуда вы сможете, безопасно извлечь карту из приставки. Для того, чтобы из {% include abbr/uk/hekate.txt abbr="hekate" %} запустить прошивку опять, перейдите в **Launch -> Atmosphere**. 

Так же вы можете подключить приставку к ПК кабелем, запустить DBI и нажать в нём {% include inc/uk/btn.txt btn="X" %}. Приставка определится компьютером как MTP-устройство и вы получите доступ к вашим файлам на карте памяти. 
...
**О:** Вам нужно установить драйвер exFAT. Для этого проделайте [безопасное обновление прошивки](/uk/update-fw){:target="_blank"}. Его нужно делать даже в том случае, если у вас и так стоит последняя версия взламываемой прошивки! Повторное обновление просто установит отсутствующий драйвер. 
...
**О:** Сам свитч, карта памяти, скрепка и {% include abbr/uk/host.txt abbr="хост" %}-устройство (ПК/Android/iOS с джейлбрейком/донгл) для [запуска взлома](/uk/fusee-gelee){:target="_blank"} у приставкок, подверженных уязвимости Fusée Gelée. 

Сам свитч, карта памяти, установленный чип для приставок с закрытой уязвимостью Fusée Gelée

Подробнее - [Существующие методы взлома](/uk/get-started){:target="_blank"}
{% endspoiler %}

{% spoiler Можно ли устанавливать любительские переводы?  %}
**О:** [Да](/uk/games#использование-переводов-и-модификаций){:target="_blank"}
...
**О:** Воспользуйтесь [этой инструкцией](/uk/link-account){:target="_blank"}

{% endspoiler %}

{% spoiler Не удаляется игра после установки, не могу удалить и через NX-shell %}
**О:** {% include /inc/uk/fixatributes.txt %}

{% endspoiler %}

{% spoiler Смогу ли я после взлома запускать сторонние программы и эмуляторы? %}
**О:** Да! {% include abbr/uk/cfw.txt abbr="Кастомная прошивка" %} поддерживает Homebrew Launcher. Причём пользоваться Homebrew Launcher можно и на SX OS без покупки лицензии.

{% endspoiler %}

{% spoiler Почему после выключения моя приставка разряжается? %}
**О:** Если у вас включен режим [AutoRCM](/uk/autorcm){:target="_blank"}, то иногда можно случайно включить приставку и без проброса {% include abbr/uk/payload.txt abbr="пейлоада" %} она застрянет в RCM и будет терять заряд. Вы можете после выключения приставки зажать и удерживать на 20 секунд кнопку питания, приставка выключится наверняка, либо отключать её через {% include abbr/uk/hekate.txt abbr="hekate" %}, выбрав Power Off. 

{% endspoiler %}

{% spoiler Не могу включить приставку %}
**О:** Отключите приставку от источника питания\вытащите из неё USB, 30 секунд удерживайте кнопку {% include inc/uk/btn.txt btn="POWER" %}, после этого нажмите на кнопку {% include inc/uk/btn.txt btn="POWER" %} единожды и запускайте прошивку. Если не помогает, снова 30 секунд удерживайте кнопку {% include inc/uk/btn.txt btn="POWER" %}, ставьте консоль на зарядку не менее, чем на 3 часа в оригинальный док или ЗУ, после чего отключите приставку от источника питания\вытащите из неё USB, 30 секунд удерживайте кнопку {% include inc/uk/btn.txt btn="POWER" %}, после этого нажмите на кнопку {% include inc/uk/btn.txt btn="POWER" %} единожды и запускайте прошивку.
...
**О:** Используйте [DBI](/uk/games){:target="_blank"} для установки
...
**О:** Да, если настроить приставку особым образом и сделать на ней [EmuNAND](/uk/emunand){:target="_blank"} 
...
**О:** Зависит от того, как чётко вы следуете инструкциям. Обязательно сделайте [бекап](/uk/backup-nand){:target="_blank"}! 
{% endspoiler %}
